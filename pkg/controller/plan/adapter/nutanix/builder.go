...
// setCDICACerts stores the trusted CA for CDI HTTP imports. CDI's Go client
// reads any PEM file under /certs/, while nbdkit curl only honors tls.crt.
...
func (r *Builder) elementHTTPSource(baseURL, name string, secret *core.Secret, configMap *core.ConfigMap, client *Client) (*cdi.DataVolumeSourceHTTP, error) {
	...
	return &cdi.DataVolumeSourceHTTP{
		URL:           fmt.Sprintf("%s/api/nutanix/v3/images/%s/file", baseURL, imageUUID(entity)),
...
func (r *Builder) resolveCentralDownload(client *Client, imageName string) (downloadURL, cookie string, err error) {
	...
	downloadURL, cookie, err = client.resolveImageV4DownloadURL(imageV4ExtID(entity))
	if err != nil {
		return "", "", err
	}
...
func (r *Builder) RefreshImportCredentials(dv *cdi.DataVolume) (bool, error) {
	...
	imageName := migrationImageName(ref.Ref{ID: vmID}, diskUUID)
